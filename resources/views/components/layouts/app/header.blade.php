...
<header class="bg-white dark:bg-gray-800 shadow-sm z-20 border-b border-gray-200 dark:border-gray-700">
    <div class="flex items-center justify-between h-16 px-4">
        ...
        <div class="flex items-center">
            ...
            <div class="ml-4 font-semibold text-xl text-blue-600 dark:text-blue-400">Sabbatical Tracker</div>
        </div>

        <!-- Right side: Search, notifications, profile -->
        <div class="flex items-center space-x-4">
            <!-- Pending Approvals Notification (Admin Only) -->
            @if(auth()->user()->isAdmin())
            @php
                $pendingCount = App\Models\Sabbatical::where('approval_status', 'pending')->count();
            @endphp
            @if($pendingCount > 0)
            <div class="relative">
                <a href="{{ route('sabbaticals.index') }}" class="flex items-center p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                    <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM4 19h6l-6 6v-6zM4 13h6l-6 6v-6zM4 7h6l-6 6V7zM10 7h10l-10 10V7z" />
                    </svg>
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="flex items-center focus:outline-none">
                    ...
                    <span class="ml-2 hidden md:block text-xs px-2 py-1 rounded-full 
                        @if(Auth::user()->isAdmin()) bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200
                        @else bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 @endif">
                        {{ Auth::user()->isAdmin() ? 'Admin' : 'Doctor' }}
                    </span>
